...
# Initialize pygame for playing warning sounds and happy sounds
pygame.mixer.init()
warning_sound = pygame.mixer.Sound('warning.wav')  # Replace 'warning.wav' with your sound file
happy_sound = pygame.mixer.Sound('happy.wav')      # Replace 'happy.wav' with your sound file
...
# Load the YOLO model
model = YOLO("best_v8_50.pt")  # Path to the pre-trained YOLOv8 model
...
# Function to log no-helmet detections
log_file = 'no_helmet_log.csv'

def log_no_helmet_detections(no_helmet_count):
    if no_helmet_count > 0:
        with open(log_file, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([datetime.now(), no_helmet_count])

# Ensure the log file exists and has a header
if not os.path.isfile(log_file):
    with open(log_file, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Timestamp", "No Helmet Count"])
...
def video_frame_callback(frame):
    ...
    # Play warning sound and log if no helmets are detected
    if no_helmet_count > 0:
        pygame.mixer.Sound.play(warning_sound)
        log_no_helmet_detections(no_helmet_count)
    else:
