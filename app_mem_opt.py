...
def video_frame_callback(frame):
    ...
    img = cv2.resize(img, (320, 240))  # Resize to a smaller resolution (320x240)
    ...
    for detection in results[0].boxes.data.tolist():
        ...
        if draw_boxes:
            cv2.rectangle(img, (x1, y1), (x2, y2), color, 1)  # Reduce line thickness to 1
        elif draw_circles:
            center = ((x1 + x2) // 2, (y1 + y2) // 2)
            radius = (x2 - x1) // 2
            cv2.circle(img, center, radius, color, 1)  # Reduce line thickness to 1

    # Display the counts on the frame
    cv2.putText(img, f"Helmet Count: {helmet_count}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, helmet_color_bgr, 1)  # Reduce font size and line thickness
    cv2.putText(img, f"No Helmet Count: {no_helmet_count}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, no_helmet_color_bgr, 1)  # Reduce font size and line thickness
